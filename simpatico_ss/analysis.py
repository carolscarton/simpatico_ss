        ...
        if any([s.lower() in self.cc for s in self.sentence]):
            ...
            for s in self.sentence:
                if (c>0) and (s.lower() =="if"):
                    ...
                    c+=1
                    if s.lower() in self.cc and "this was " + s.lower() not in " ".join(self.sentence).lower() and "this is " + s.lower() not in " ".join(self.sentence).lower() and "this happens " + s.lower() not in " ".join(self.sentence).lower() and "this happened " + s.lower() not in " ".join(self.sentence).lower() :
