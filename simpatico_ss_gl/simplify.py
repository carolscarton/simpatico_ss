    ...
    def transformation(self, sent, ant, justify=False):
        ...
        def appositive_phrases(aux, words, root, deps_root, ant):
            ...
            if 'nsubj' in deps_root.keys():
                ...
                if 'nmod' in deps_subj: ## bug -- this generates several mistakes... 
                    mod = deps_subj['nmod'][0]
                    if mod in aux:
                        deps_mod = aux[mod]
                    else:
                        deps_mod = {}
                    del aux[subj]['nmod']
                    deps_root['nsubj'][0] = mod
                    deps_subj = aux[subj]
                ...
                sentence1, sentence2 = self.generation.print_sentence_appos(final_root, final_subj, final_appos, v_tense, n_num, subj_word)
        ...
        def passive_voice(aux, words, root, deps_root, ant):
            ...
            @return: a flag that indicates whether or not the sentence was simplified and the result sentence (if flag = False, ant is returned)
            """
            if 'case' in deps_root.keys() and 'aux' in deps_root.keys():
                case = deps_root['case'][0]
                v = deps_root['aux'][0]
                if case in aux and v in aux:
                    deps_case = aux[case]
                    deps_v = aux[v]
                    if words[case-1][0].lower() == 'por' or words[case-1][0].lower() == 'polos':
                        if 'nmod' in deps_case.keys():
                            subj = deps_case['nmod'][0]
                            subj_tag = words[subj-1][1]['PartOfSpeech']
                            subj_word = words[subj-1][0]
                            if subj in aux:
                                deps_subj = aux[subj]
                            else:
                        ...
                        v_aux = words[root-1][1]['PartOfSpeech']
                        ...
                        build(subj, deps_subj, aux, words, final_subj)
                        final_obj = {}
                        build(v, deps_v, aux, words, final_obj, False)
                        final_root = {}
                        build(root, deps_root, aux, words, final_root, False)

                        sentence1 = self.generation.print_sentence_voice(final_subj, final_obj, words[root-1][0], v_aux, aux_tense, subj_tag, subj_word, final_root=final_root)
                        s1 = self.transformation(sentence1, ant)
                        return True, s1
                    else:
                        return False, ant
        ...
        """ Conjoint clauses and relative clauses are not supported due to parser restrictions.
